...
void CBubble::SetState(const _vec3& pStartPos,float fLifeTime)
{	
	this->SetLifeTime(fLifeTime);
	m_pTransform->SetPosition(pStartPos); 
	this->SetMoveDir(_vec3(0.f, RAND_RANGE_F(0.f,0.01f) ,0.f));
}
...
HRESULT CBubble::Init()
{
	...
	this->SetColor(_vec4( 1.f , 1.f , 1.f, 0.5f));
	this->SetTargetColor(_vec4( 1.f , 1.f , 1.f, 0.f));

	m_pTransform->SetScale(_vec3(RAND_RANGE_F(1.f,1.3f), RAND_RANGE_F(1.f,1.3f), 0.1f));
	m_pAnimator->SetPlaySpeed(RAND_RANGE_F(0.8f,1.2f));

	this->Play(RESOURCE_STATIC, "TEX_BUBBLE"); //TEX_BUBBLE_EX
